...
static void setup_mbuf_indir(void)
{
	for (int i = 0; i < MBUF_NUM; i++) {
		...
		tx_packets[i]->pkt_len += tx_packets_indir[i]->pkt_len;
		tx_packets[i]->nb_segs++;
		tx_packets[i]->next = tx_packets_indir[i];
	}
}

static void
setup_mbuf(uint8_t pid_from, uint8_t pid_to, int cnt)
{
	char *pkt;
...
static void
construct_2_segment_mbufs(struct rte_mbuf **mbufs, uint16_t cnt,
		uint8_t pid_from, uint8_t pid_to)
{
	setup_mbuf(pid_from, pid_to, cnt);
	...
	for (int i = 0; i < cnt; i++) {
		...
		rte_pktmbuf_reset_headroom(mbufs[i]);
		rte_pktmbuf_attach(mbufs[i], tx_packet0);

		tx_packets[i]->pkt_len += mbufs[i]->pkt_len;
		tx_packets[i]->nb_segs++;
		tx_packets[i]->next = mbufs[i];
	}
}
...
begin:
	...
	for (;;) {
		...
		construct_2_segment_mbufs(tx_packets_indir, MBUF_NUM,
				pid_from, pid_to);
		nb_tx = rte_eth_tx_burst(pid_from, queue_id, tx_packets, MBUF_NUM);
		statistic.tx_pkts += nb_tx;
		//rte_eth_stats_get(pid_from, &stats);
...
int main(int argc, char **argv)
{
	...
	init_mempool();
	setup_mbuf(pid_from, pid_to, MBUF_NUM);
	init_mempool_indir();
	setup_mbuf_indir();
	...
	//const struct rte_eth_txconf *tx_conf = &dev_info.default_txconf;
